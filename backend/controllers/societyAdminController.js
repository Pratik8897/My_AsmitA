...
exports.createSociety = async (req, res) => {
  try {
    const { society_name, email, phone } = req.body;

    const [result] = await db.query(
      `INSERT INTO societies (society_admin_name, society_name, email, phone)
       VALUES (?, ?, ?, ?)`,
      [society_admin_name, society_name, email, phone]
    );
...
exports.updateSociety = async (req, res) => {
  try {
    const { id } = req.params;
    const { society_admin_name, society_name, email, phone } = req.body;

    await db.query(
      `UPDATE societies 
       SET society_admin_name = ?, society_name = ?, email = ?, phone = ?
       WHERE society_id = ?`,
      [society_admin_name, society_name, email, phone, id]
    );
